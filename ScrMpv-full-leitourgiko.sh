    ...
    if [ "$option" == "video" ]; then
        ...
        select opt in $SecondOpt; do
            if [ "$opt" == "record" ]; then
                ...
                mpv --cache=yes --stream-record="$name".mp4 --ytdl-format=bestvideo[height "$link" <=?1080]+bestaudio/best
                mpv $name.mp4 -o $name.mp3 
                rm $name.mp4
                break
            elif [ "$opt" == "No" ]; then
                echo " Now playing "$(youtube-dl "$link" --get-title)""
        ...
        select opt in $SecondOpt; do
            if [ "$opt" == "record" ]; then
                ...
                read -r name
                echo "Now playing "$(youtube-dl "$link" --get-title)""
                mpv --cache=yes --video=no --stream-record="$name".mp4 --ytdl-format=bestaudio/best "$link"
                mpv $name.mp4 -o $name.mp3 --no-video
                rm $name.mp4
                break
            elif [ "$opt" == "No" ]; then
                echo "Now playing "$(youtube-dl "$link" --get-title)""
